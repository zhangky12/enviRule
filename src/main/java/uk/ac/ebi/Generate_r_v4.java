...
public class Generate_r_v4{

    static boolean generalizeIgnoreHydrogen = true;
    static boolean includeFunctionalGroups = true;
    ...
    public static void main(String[] args) throws Exception{
        ...
        String file_name = "/Users/kunyang/Documents/Eawag/envipath/envipath-python/enviPath_python/BBD_reactions/Reactions4Rules/99-15.txt"; // 87-39 18-4
...
//                System.out.println("Complete: " + connected_rule);
        ...
        // TODO: finish this part. Store the change of atom mapping.
...
//            String complete_rule = completeWithHydrogen(base_rule, new_rule, smilesParser);
            if (coa_count ==0 || coa_count < (reactions.size()/2)){
//                String complete_rule = completeWithHydrogen2(base_rule, new_rule, atomMappingReactions.get(i), atom_map_list.get(i), smilesParser);
                ...
                // Do not unmap unrelated atoms. Not yet
//            complete_rule = unmapUnrelatedAtoms(complete_rule);
...
//        System.out.println("------------------------------------------");
    ...
    public static boolean checkCoA(String base_rule) throws CDKException {
    ...
    public static String connectCenters(IReaction performAtomAtomMapping, SmilesGenerator sg, SmilesParser sp, int radius) throws Exception {
...
//        return connected_rule;
    ...
    public static void permuteHelper(List<List<Integer>> list, List<Integer> resultList, List<Integer> arr){
    ...
    public static void BFSDistance(int[][] dynamic_table, IAtomContainer compound, Map<Integer, Integer> ids2index, IAtom atom, Set<Integer> visited){
    ...
    public static String unmapUnrelatedAtoms(String complete_rule){
     ...
     * @return
     */
    public static Map<Integer, String> getConditions(List<String> segments, Map<String, String> conditions_inchiKey, SmilesParser smilesParser) throws CDKException {
    ...
    public static Set<Integer> getChangedAtoms(IReaction performAtomAtomMapping, int radius) throws Exception{
        // Get changed atoms
     ...
     * @throws Exception
     */
    public static String getNewRule(IReaction performAtomAtomMapping, SmilesGenerator sg, int radius, Boolean unmapUnrelated) throws Exception {
    ...
    public static String generalizedForm(String base_rule, IReaction performAtomAtomMapping, SmilesParser smilesParser, SmilesGenerator sg, int radius, boolean unmapUnrelated) throws Exception {
    ...
    public static Set<Integer> uniqueMapId(String reactant_smart, String product_smart){
    ...
    public static String unmapUnrelated(IAtomContainer reactant_fragments, IAtomContainer product_fragments, String rule, Set<Integer> changed_atom_tags){
...
//        reactant_smart = replaceWithExplicitBond(reactants, reactant_smart, false, true);
//        product_smart = replaceWithExplicitBond(products, product_smart, false, false);
        reactant_smart = replaceWithExplicitBond(performAtomAtomMapping, reactant_smart, false, true);
        product_smart = replaceWithExplicitBond(performAtomAtomMapping, product_smart, false, false);
        String new_smirks = reactant_smart + ">>" + product_smart;

        return new_smirks;
    }

    public static String replaceWithExplicitBond(IReaction performAtomAtomMapping, String smart, boolean fragments, boolean isReactant) throws CDKException {

        Aromaticity aromaticity = new Aromaticity(ElectronDonation.daylight(), Cycles.or(Cycles.all(), Cycles.edgeShort()));
        IAtomContainerSet compounds = null;

        if(isReactant){
            compounds = performAtomAtomMapping.getReactants();
            return replaceWithExplicitBond(compounds, smart, fragments, isReactant);
        }else{
            IAtomContainerSet reactants = performAtomAtomMapping.getReactants();
            IAtomContainerSet products = performAtomAtomMapping.getProducts();
            compounds = products;

            Set<IBond> produced_aromatic_bonds = new HashSet<>();

            Set<String> reactants_aromatic_bonds = new HashSet<>();
            for(int i=0; i<reactants.getAtomContainerCount(); i++){
                IAtomContainer reactant = reactants.getAtomContainer(i);
                aromaticity.apply(reactant);
                for (IBond bond: reactant.bonds()){
                    if (bond.isAromatic()){
                        if (bond.getBegin().getMapIdx() < bond.getEnd().getMapIdx()){
                            reactants_aromatic_bonds.add(bond.getBegin().getMapIdx() + "+" + bond.getEnd().getMapIdx());
                        }else{
                            reactants_aromatic_bonds.add(bond.getEnd().getMapIdx() + "+" + bond.getBegin().getMapIdx());
                        }
                    }
                }
            }
            ...
            for(int i=0; i<products.getAtomContainerCount(); i++){
                ...
                for (IBond bond: product.bonds()){
                    if (bond.isAromatic()){
                        String aromatic_bonds_code = "";
                        if (bond.getBegin().getMapIdx() < bond.getEnd().getMapIdx()){
                            aromatic_bonds_code = bond.getBegin().getMapIdx() + "+" + bond.getEnd().getMapIdx();
                        }else{
                            aromatic_bonds_code = bond.getEnd().getMapIdx() + "+" + bond.getBegin().getMapIdx();
                        }
                        if (!reactants_aromatic_bonds.contains(aromatic_bonds_code)){
                            // This aromatic bond is produced
                            produced_aromatic_bonds.add(bond);
                        }
                    }
                }
            }
...
//                            if(bond.isAromatic()){
                                ...
                                if(bond.isAromatic() && aromacitySanityCheck(bond, mol) && !produced_aromatic_bonds.contains(bond)){
                                    replace_string = ":" + segments.get(i);
                                }else if(bond.isAromatic() && produced_aromatic_bonds.contains(bond)){
                                    // For all generated aromatic bonds, denote them with "="
                                    replace_string = "=" + segments.get(i);
                                }else{
    ...
    public static boolean aromacitySanityCheck(IBond aro_bond, IAtomContainer compound){
     ...
     * @throws CDKException
     */
    public static String replaceWithExplicitBond(IAtomContainerSet compounds, String smart, boolean fragments, boolean isReactant) throws CDKException {
...
//                            if(bond.isAromatic()){
    ...
    public static void getSegmentsWithID(String smart, List<String> segments, List<Integer> segments_mapId){
    ...
    public static void correctAromaticBondInRing(IAtomContainer mol, IAtomContainerSet wholeMolecule) throws CDKException {
    ...
    public static String completeWithHydrogen(String expanded_smirks, IReaction performAtomAtomMapping, SmilesParser smilesParser) throws CDKException {
        ...
        for(IAtomContainer reactant_mol: reactants_mols){
            for(IAtom atom: reactant_mol.atoms()){
                if(atom.getMapIdx() != 0){
                    ...
                    // TODO:First check whether the atom is charged
                    // Right now, only deal with negative charge
                    if(atom.getFormalCharge() < 0) bond_orders -= atom.getFormalCharge();

                    for(IBond bond: reactant_mol.getConnectedBondsList(atom)){

                        boolean found_bond = false;
                        IBond correponding_bond = null;
                        for (int i=0; i<performAtomAtomMapping.getReactants().getAtomContainerCount(); i++){
                            IAtomContainer reactant = performAtomAtomMapping.getReactants().getAtomContainer(i);
                            aromaticity.apply(reactant);
                            for (IBond orig_bond: reactant.bonds()){
                                if ((orig_bond.getBegin().getMapIdx() == bond.getBegin().getMapIdx() && orig_bond.getEnd().getMapIdx() == bond.getEnd().getMapIdx()) || (orig_bond.getEnd().getMapIdx() == bond.getBegin().getMapIdx() && orig_bond.getBegin().getMapIdx() == bond.getEnd().getMapIdx())){
                                    if (orig_bond.getOrder() == IBond.Order.DOUBLE || orig_bond.isAromatic()){
                                        bond_orders += 2;
                                    }else if(orig_bond.getOrder() == IBond.Order.SINGLE){
                                        bond_orders += 1;
                                    }else if(orig_bond.getOrder() == IBond.Order.TRIPLE){
                                        bond_orders += 3;
                                    }else{
                                        // Haven't seen quad bonds yet
                                    }
                                    found_bond = true;
                                    correponding_bond = orig_bond;
                                    break;
                                }
                            }
                            if(found_bond) break;
                        }
                        if(!found_bond) System.exit(2);
                        ...
                        if((atom.getAtomicNumber() == 16 || atom.getAtomicNumber() == 15)
                                ...
                                && correponding_bond.getOrder() == IBond.Order.DOUBLE){
                            bond_orders -= 2;
                        }
                    }
                ...
                if(atom.getMapIdx() != 0){
                    ...
                    for(IBond bond: product_mol.getConnectedBondsList(atom)){
                        ...
                        for (int i=0; i<performAtomAtomMapping.getProducts().getAtomContainerCount(); i++){
                            ...
                            aromaticity.apply(product);
                            for (IBond orig_bond: product.bonds()){
                                if ((orig_bond.getBegin().getMapIdx() == bond.getBegin().getMapIdx() && orig_bond.getEnd().getMapIdx() == bond.getEnd().getMapIdx()) || (orig_bond.getEnd().getMapIdx() == bond.getBegin().getMapIdx() && orig_bond.getBegin().getMapIdx() == bond.getEnd().getMapIdx())){
                                    if (orig_bond.getOrder() == IBond.Order.DOUBLE || (orig_bond.isAromatic() && aromacitySanityCheck(orig_bond, product))){
                                        bond_orders += 2;
                                    }else if(orig_bond.getOrder() == IBond.Order.SINGLE){
                                        bond_orders += 1;
                                    }else if(orig_bond.getOrder() == IBond.Order.TRIPLE){
    ...
    public static String completeWithHydrogen3(String smirks, String expanded_smirks, IReaction performAtomAtomMapping, Map<String, String> atom_map, SmilesParser smilesParser) throws CDKException {
    ...
    public static String completeWithHydrogen2(String smirks, String expanded_smirks, IReaction performAtomAtomMapping, Map<String, String> atom_map, SmilesParser smilesParser) throws CDKException {
            ...
            for(IAtom atom: reactant_mol.atoms()){
                if(atom.getMapIdx() != 0){
                    ...
                    for(IBond bond: reactant_mol.getConnectedBondsList(atom)){
                        ...
                        if(reverse_mapIds.containsKey(bond.getBegin().getMapIdx())){
                            start = reverse_mapIds.get(bond.getBegin().getMapIdx());
                        }else{
                            start = bond.getBegin().getMapIdx();
                        }

                        if(reverse_mapIds.containsKey(bond.getEnd().getMapIdx())){
                            end = reverse_mapIds.get(bond.getEnd().getMapIdx());
                        }else{
                            end = bond.getEnd().getMapIdx();
                        }
                        ...
                        for (int i=0; i<performAtomAtomMapping.getReactants().getAtomContainerCount(); i++){
                            ...
                            aromaticity.apply(reactant);
                            for (IBond orig_bond: reactant.bonds()){
                                if ((orig_bond.getBegin().getMapIdx() == start && orig_bond.getEnd().getMapIdx() == end) || (orig_bond.getEnd().getMapIdx() == start && orig_bond.getBegin().getMapIdx() == end)){
                                    if (orig_bond.getOrder() == IBond.Order.DOUBLE || orig_bond.isAromatic()){
                                        bond_orders += 2;
                                    }else if(orig_bond.getOrder() == IBond.Order.SINGLE){
                                        bond_orders += 1;
